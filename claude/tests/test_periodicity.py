...
def test_intramonth_flows_pool_onto_that_month_end():
    path = fund(**HISTORY).on(Timeline(MONTH_ENDS))
    np.testing.assert_allclose(path.calls, [0, 0.6, 0.05, 0, 0.1, 0])  # 3, 15 and 28 Feb → 28 Feb; 1 Mar → 31 Mar
    np.testing.assert_allclose(path.distributions, [0, 0, 0, 0.04, 0, 0.06])  # a flow on a month end stays there
    np.testing.assert_allclose(path.nav, [0, 0.6, 0.7, 0.66, 0.76, 0.7])  # the 20 Mar mark is what 31 Mar sees


def test_pooling_is_consistent_across_frequencies():
    monthly = fund(**HISTORY).on(Timeline(MONTH_ENDS))
    quarterly = fund(**HISTORY).on(Timeline(QUARTER_ENDS))
    np.testing.assert_allclose(quarterly.calls, [monthly.calls[:3].sum(), monthly.calls[3:].sum()])
    np.testing.assert_allclose(quarterly.distributions, [monthly.distributions[:3].sum(), monthly.distributions[3:].sum()])
    np.testing.assert_allclose(quarterly.nav, monthly.nav[[2, 5]])
    single = fund(**HISTORY).on(Timeline([MONTH_ENDS[-1]]))
    np.testing.assert_allclose(single.calls, [monthly.calls.sum()])
    np.testing.assert_allclose(single.nav, [monthly.nav[-1]])
...
def test_daily_grid_moves_weekend_events_to_the_next_pricing_day():
    ...
    path = Fund("W", "VC", "2027-01-01", unit_calls=[("2027-01-09", 0.5)]).on(business_days)  # a Saturday
    assert list(business_days.dates[np.flatnonzero(path.calls)]) == [pd.Timestamp("2027-01-11")]  # the Monday
...
def test_irregular_grid_uses_the_same_rule():
    ...
    path = Fund("I", "VC", "2027-01-05", unit_calls=history).on(grid)
    np.testing.assert_allclose(path.calls, [0, 0.1, 0.5, 0.4])
...
def test_quarterly_grid_with_monthly_marks_uses_the_last_mark_then_adjusts_for_flows():
    ...
    path = f.on(Timeline(QUARTER_ENDS))
    np.testing.assert_allclose(path.nav, [0.60, 0.50])  # 0.65 mark then −0.05; then −0.10 with no newer mark
...
def test_event_map_shows_where_each_event_landed():
    ...
    events = Simulator(month_end_portfolio(), [f]).event_map()
    ...
    assert Simulator(month_end_portfolio()).event_map().empty


def test_timeline_assign_matches_index_of():
    timeline = Timeline(MONTH_ENDS)
    days = ["2027-01-31", "2027-02-01", "2027-02-28", "2027-07-01"]
    assert timeline.assign(days).tolist() == [timeline.index_of(d) for d in days] == [0, 1, 1, 6]
...
def test_exchange_rates_carry_forward_while_flows_roll_forward():
    ...
    np.testing.assert_allclose(timeline.asof(rates), [0.8, 0.8, 0.9, 0.9, 0.9, 0.9])
    assert timeline.assign(["2027-03-10"]).tolist() == [2]
